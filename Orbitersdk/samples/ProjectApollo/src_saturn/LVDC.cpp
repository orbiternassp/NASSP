			...
			case 0: // MORE TB0
				...
				if (lvCommandConnector->GetMissionTime() < -0.715) {
					...
					if (SumThrust > 0) { //let's hope that those numberings are right...
						...
						lvCommandConnector->AddForce(_V(0, 0, -10. * lvCommandConnector->GetFirstStageThrust()), _V(0, 0, 0)); // Maintain hold-down lock
					}
				}
				else {
					...
					lvCommandConnector->AddForce(_V(0, 0, -10. * lvCommandConnector->GetFirstStageThrust()), _V(0, 0, 0));
				}
