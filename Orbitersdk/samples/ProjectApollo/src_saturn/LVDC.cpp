...
void LVDCSV::TimeStep(double simt, double simdt) {
	...
	//					MissionTime,LVDC_TB_ETime,thrst[0],thrst[1],thrst[2],SumThrust);
			...
			case 6:
				...
				if (LVDC_TB_ETime > 497.3 && GATE7 == false && !INH)
				{
					GATE7 = true;
				}
				...
				//CSM separation detection
				if (lvCommandConnector->CSMSeparationSensed() && LVDC_TB_ETime < 560.0 && TB5a > 99999.9)
				{
					TB5a = TAS;
					LVDC_TB_ETime = 0;
					LVDC_Timebase = 50;
...
restartprep:
		...
		INHcheck:
			...
			else if (INH)	//XLUNAR switch to INHIBIT in the CSM?
			{
				if (first_op)
				{
