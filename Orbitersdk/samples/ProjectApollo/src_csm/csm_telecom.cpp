...
void VHFRangingSystem::TimeStep(double simdt)
{
	...
	if (val13[RangeUnitActivity] == 1) {
		...
		case 4:
			...
			sat->agc.GenerateInterrupt(ApolloGuidance::Interrupt::RADARUPT);
			break;
		default:
			break;
		}
...
void PCM::generate_stream_lbr(){
	...
	switch(word_addr){
		case 0: // SYNC 1
			...
			sat->agc.GenerateInterrupt(ApolloGuidance::Interrupt::DOWNRUPT);
			// And continue
...
void PCM::handle_uplink() {
	...
	case 21: // CMC UPLINK
	{
		...
		// Cause UPRUPT
		sat->agc.GenerateInterrupt(ApolloGuidance::Interrupt::UPRUPT);
