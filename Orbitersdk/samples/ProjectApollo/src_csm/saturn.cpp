...

{
	...
	oapiWriteScenario_int (scn, "FAILURE_MODE", failuremode);
...

{
	...
	else if (!strnicmp(line, "FAILURE_MODE", 12)) {
		sscanf(line + 12, "%d", &failuremode);
	}
...

{
	...
	char* sysl = NULL;
	if (failuremode == 1) { fail_dice = rand() % 100000; sysdice = (rand() % 257) + 1; }
	if (failuremode == 2) { fail_dice = rand() % 1000; sysdice = (rand() % 257) + 1; }
	if (failuremode == 3) { fail_dice = rand() % 20; sysdice = (rand() % 257) + 1; }
