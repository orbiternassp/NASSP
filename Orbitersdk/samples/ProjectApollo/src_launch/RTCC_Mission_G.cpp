...
bool RTCC::CalculationMTP_G(int fcn, LPVOID &pad, char * upString, char * upDesc, char * upMessage)
{
	...
	case 12: //TLI+90 MANEUVER PAD
	{
		...
		AGCStateVectorUpdate(buffer1, sv, true, calcParams.TEPHEM);
	...
	case 95: //PLANE CHANGE TARGETING (FOR BURN)
	{
		...
		LunarDescentPlanningProcessor(sv, calcParams.TEPHEM, BZLAND.lat[RTCC_LMPOS_BEST], BZLAND.lng[RTCC_LMPOS_BEST], R_LLS);

		PoweredFlightProcessor(sv, calcParams.TEPHEM, PZLDPDIS.GETIG[0], RTCC_ENGINETYPE_CSMSPS, 0.0, PZLDPDIS.DVVector[0] * 0.3048, true, TimeofIgnition, DeltaV_LVLH);
		...
		else
		{
			...
			AGCStateVectorUpdate(buffer1, sv_CSM, true, calcParams.TEPHEM);
			CMCExternalDeltaVUpdate(buffer2, TimeofIgnition, DeltaV_LVLH);
