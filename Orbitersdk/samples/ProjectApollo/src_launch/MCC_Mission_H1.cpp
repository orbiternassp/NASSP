...
void MCC::MissionSequence_H1()
{
	...
	case MST_H1_TRANSLUNAR9: //Block Data 2 to MCC-2 update
		...
		break;
	case MST_H1_TRANSLUNAR10: //MCC-2 update to Lunar Flyby PAD
		UpdateMacro(UTP_PADWITHCMCUPLINK, PT_AP11MNV, rtcc->GETEval2(rtcc->calcParams.TLI + 32.0*3600.0 + 10.0*60.0), 22, MST_H1_TRANSLUNAR11);
		break;
	case MST_H1_TRANSLUNAR11: //Lunar Flyby PAD to MCC-3
		UpdateMacro(UTP_PADONLY, PT_AP11MNV, rtcc->GETEval2(rtcc->calcParams.LOI - 24.0*3600.0 - 40.0*60.0), 23, MST_H1_TRANSLUNAR12);
		break;
	case MST_H1_TRANSLUNAR12: //MCC-3 update to MCC-4 update
		UpdateMacro(UTP_PADWITHCMCUPLINK, PT_AP11MNV, rtcc->GETEval2(rtcc->calcParams.LOI - 6.0*3600.0 - 25.0*60.0), 24, MST_H1_TRANSLUNAR13);
		break;
	case MST_H1_TRANSLUNAR13: //MCC-4 update to PC+2 update
		UpdateMacro(UTP_PADWITHCMCUPLINK, PT_AP11MNV, SubStateTime > 5.0*60.0, 25, MST_H1_TRANSLUNAR14);
		break;
	case MST_H1_TRANSLUNAR14: //PC+2 update to LOI-1 update
		UpdateMacro(UTP_PADONLY, PT_AP11MNV, rtcc->GETEval2(rtcc->calcParams.LOI - 2.0*3600.0 - 50.0*60.0), 26, MST_H1_TRANSLUNAR15);
		break;
	case MST_H1_TRANSLUNAR15: //LOI-1 update to TEI-1 update
		UpdateMacro(UTP_PADWITHCMCUPLINK, PT_AP11MNV, rtcc->GETEval2(rtcc->calcParams.LOI - 2.0*3600.0 - 5.0*60.0), 30, MST_H1_TRANSLUNAR16);
		break;
	case MST_H1_TRANSLUNAR16: //TEI-1 update to TEI-4 update
		UpdateMacro(UTP_PADONLY, PT_AP11MNV, SubStateTime > 5.0*60.0, 40, MST_H1_TRANSLUNAR17);
		break;
	case MST_H1_TRANSLUNAR17: //TEI-4 update to Rev 1 Map Update
		UpdateMacro(UTP_PADONLY, PT_AP11MNV, rtcc->GETEval2(rtcc->calcParams.LOI - 40.0*60.0), 41, MST_H1_TRANSLUNAR18);
		break;
	case MST_H1_TRANSLUNAR18: //Rev 1 Map Update to lunar orbit phase begin
		UpdateMacro(UTP_PADONLY, PT_AP10MAPUPDATE, rtcc->GETEval2(rtcc->calcParams.LOI), 60, MST_H1_LUNAR_ORBIT_LOI_DAY_1);
		break;
