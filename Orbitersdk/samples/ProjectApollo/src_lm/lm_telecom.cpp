	...
	switch(conn_state){
		...
		case 1: // INITALIZED, LISTENING
			...
			if (mcc_size > 0) {
				...
				// Should we recieve?
				if ((fabs(simt - last_rx) / 0.1) < 1 || lem->agc.InterruptRequested(ApolloGuidance::Interrupt::UPRUPT)) {
					return; // No
				}
				last_rx = simt;
				// Yes. Take a byte
		...
		case 2: // CONNECTED			
			...
			// Should we receive?
			if (((simt - last_rx) / 0.005) < 1 || lem->agc.InterruptRequested(ApolloGuidance::Interrupt::UPRUPT)) {
				return; // No
			}
			last_rx = simt;
...
void LM_PCM::handle_uplink()
{
	...
	case 21: // LGC UPLINK
	{
		...
		// Cause UPRUPT
		lem->agc.GenerateInterrupt(ApolloGuidance::Interrupt::UPRUPT);
...
void LM_PCM::generate_stream_hbr(){
	...
	switch(word_addr){
		case 0: tx_data[tx_offset] = 0375;										// SYNC 1
			lem->agc.GenerateInterrupt(ApolloGuidance::Interrupt::DOWNRUPT);	// And generate DOWNRUPT (We read the data out at 120)
			break;
