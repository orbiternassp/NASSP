...

{
	...
	case AID_RR_NOTRACK:
		if(RR.IsDCPowered() && !RR.IsRadarDataGood() || LampToneTestRotary.GetState() == 6){ // The AC side is only needed for the transmitter
	...
	case AID_CO2_LIGHT:
		if (ECS_CO2_SENSOR_CB.IsPowered() && (scera1.GetVoltage(5, 2) >= (7.6 / 6) || CO2CanisterSelectSwitch.GetState() == 0 || LampToneTestRotary.GetState() == 6)) {
			oapiBlt(surf, srf[SRF_RR_NOTRACK], 0, 0, 0, 34, 34, 34, SURF_PREDEF_CK); // Light On
		}
		else {
			oapiBlt(surf, srf[SRF_RR_NOTRACK], 0, 0, 0, 0, 34, 34, SURF_PREDEF_CK); // Light Off
	...
	case AID_H2OSEP_LIGHT:
		if (scera1.GetVoltage(5, 3) < 1.1 || LampToneTestRotary.GetState() == 6) {
			oapiBlt(surf, srf[SRF_RR_NOTRACK], 0, 0, 0, 34, 34, 34, SURF_PREDEF_CK); // Light On
		}
		else {
			oapiBlt(surf, srf[SRF_RR_NOTRACK], 0, 0, 0, 0, 34, 34, SURF_PREDEF_CK); // Light Off
	...
	case AID_DC_BUS_LIGHT:
		if (LampToneTestRotary.GetState() == 6) {								//Needs control logic to indicate DC bus or main feeder grounded
			oapiBlt(surf, srf[SRF_RR_NOTRACK], 0, 0, 0, 34, 34, 34, SURF_PREDEF_CK); // Light On
		}
		else {
			oapiBlt(surf, srf[SRF_RR_NOTRACK], 0, 0, 0, 0, 34, 34, SURF_PREDEF_CK); // Light Off
	...
	case AID_BAT_FAULT_LIGHT:
		if (LampToneTestRotary.GetState() == 6) {								//Needs control logic to indicate overcurrent, reverse current, or over temp of selected battery
			oapiBlt(surf, srf[SRF_RR_NOTRACK], 0, 0, 0, 34, 34, 34, SURF_PREDEF_CK); // Light On
		}
		else {
			oapiBlt(surf, srf[SRF_RR_NOTRACK], 0, 0, 0, 0, 34, 34, SURF_PREDEF_CK); // Light Off
