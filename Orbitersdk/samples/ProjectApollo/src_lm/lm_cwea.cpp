...
void LEM_CWEA::TimeStep(double simdt) {
	...
	if (lem->CDR_LTG_ANUN_DOCK_COMPNT_CB.Voltage() > SP_MIN_DCVOLTAGE) {
		if (IsCWEAPowered()) {
			...
			LightStatus[3][1] = 0;
			if ((val163[Ch163DSKYWarn]) && lem->GuidContSwitch.GetState() == TOGGLESWITCH_UP) { LightStatus[3][1] = 1; }

			// 6DS10 ISS FAILURE
			// On when ISS power supply fails, PIPA fails while main engine thrusting, gimbal servo fails, CDU fails.
			// Disabled by Guidance Control switch in AGS position.
			LightStatus[4][1] = 0;
			if ((val11[ISSWarning] || val33[PIPAFailed] || val30[IMUCDUFailure] || val30[IMUFailure]) && lem->GuidContSwitch.GetState() == TOGGLESWITCH_UP) { LightStatus[4][1] = 1; }
			...
			LightStatus[0][4] = 0;
			if (lem->APSPropellant.GetHeliumRegulator1OutletPressurePSI() > 220.0) { LightStatus[0][4] = 1; }
			...
			LightStatus[1][4] = 0;
			if (lem->ph_Asc && lem->APS.thrustOn && lem->GetPropellantFlowrate(lem->ph_Asc) > 0.0 && (lem->GetPropellantMass(lem->ph_Asc) / lem->GetPropellantFlowrate(lem->ph_Asc) < 10.0)) { LightStatus[1][4] = 1; }
			...
			LightStatus[2][4] = 0;
			if (lem->stage < 2 && (abs(lem->DPS.pitchGimbalActuator.GetPosition()) >= 6.0 || abs(lem->DPS.rollGimbalActuator.GetPosition()) >= 6.0)) { LightStatus[2][4] = 1; }
