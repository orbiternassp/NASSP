			...
			else
			{
				...
				if (err == 0)
				{
					...
					if (soln == 0)
					{
						...
						AnalyticEphemerisGenerator::PMMDAN(pRTCC, sv_temp, 1, PMMDAN_ERR, T_c, T_c_apo);
						if (PMMDAN_ERR)
						{
							DT_Light = 0.0;
						}
					...
					AnalyticEphemerisGenerator::PMMDAN(pRTCC, opt.RequestIndicator == 2 ? sv_C1_apo : sv_C2_apo, 1, PMMDAN_ERR, T_c, T_c_apo);
					if (PMMDAN_ERR)
					{
...
void TwoImpulseProcessor::SingleSolutionTransferPlan()
{
	...
	if (opt.mode == 3)
	{
		...
		AnalyticEphemerisGenerator::PMMDAN(pRTCC, sv_C1_apo, 1, PMMDAN_ERR, T_c1, T_c_apo);
		AnalyticEphemerisGenerator::PMMDAN(pRTCC, sv_C2_apo, 1, PMMDAN_ERR, T_c2, T_c_apo);
		//Put data in table
