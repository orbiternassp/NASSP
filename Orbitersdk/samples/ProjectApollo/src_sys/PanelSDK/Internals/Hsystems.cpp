...
ship_object* ship_system::GetSystemByName(char* r_name)
{
	...
	runner = List.next;
	while (runner) {
		if (!stricmp(runner->name, r_name)) return runner;
		runner = runner->next;
	}
	return NULL;
};
void ship_system::SetMaxStage(char* name, int stage)
{
	...
	runner = List.next;
	while (runner) {
		if (!strnicmp(runner->name, name, strlen(name)))runner->max_stage = stage;
		runner = runner->next;
	}
}
...
void H_system::ProcessShip(VESSEL* vessel, PROPELLANT_HANDLE ph)
{
	...
	while (runner) {
		((h_object*)runner)->ProcessShip(vessel, ph);
		runner = runner->next;
	}
...
double h_substance::GET_BULK_MOD(const int SUBSTANCE_TYPE, const double temperature, const double pressure) const
{
	int lookup_index = 0;
	double bulkmod = 0;
	switch (SUBSTANCE_TYPE) {
	case SUBSTANCE_O2:
		...
		{ 
			...
			//lookup is implimented by array index (spacing of 1K temperature)
			double bulkmod1, bulkmod2;
			bulkmod1 = O2_Bulk_Mod[lookup_index];
			bulkmod2 = O2_Bulk_Mod[lookup_index+1];

			bulkmod = bulkmod1 +(temperature - (double)lookup_index) * (bulkmod2 - bulkmod1);
		}
		else { bulkmod = O2_Bulk_Mod[999]; }
		
		bulkmod += 102883.9079210175 * (pressure / 6.205E+6); // pressure correction: 8.290027363170852 PSI/R reference @ 900 PSI

		if (bulkmod < 0) {bulkmod = 0;}
		break;
	default:
		bulkmod = BULK_MOD[SUBSTANCE_TYPE];
	}
	return bulkmod;
}
...
void h_volume::ThermalComps(double dt) {
	...
	for (i = 0; i < MAX_SUB; i++) {
		...
		double LiquidBulkModulus = composition->GET_BULK_MOD(i, Temp, Press);
		if(LiquidBulkModulus > 0)
			PNV += tNV / LiquidBulkModulus;	//Units of L/Pa
	}
...
void h_WaterSeparator::refresh(double dt) {
	...
	if (out->open && in->open) {
		...
		rpmcmd = flow * 418.36; // This value gives approximately 2050 RPM at 4.90 g/s flow which is what our current simulation tops off at
		...
		if (flow != 0) {
			...
			if (h2oremovalratio > 0)
			{
				...
				//if (!strcmp(name, "WATERSEP1"))
					//sprintf(oapiDebugString(), "RPM %f Rate %f Removed %f Remaining %f Flow %f", RPM, h2oremovalratio, removedmass / dt, fanned.composition[SUBSTANCE_H2O].mass / dt, flow);
			}
		}
