...
sat5_lmpkd::sat5_lmpkd(OBJHANDLE hObj, int fmodel) : LanderVessel (hObj, fmodel)
...

{
	...
	LunarAscent = LoadSound("lunar_ascent.wav");
	StageS = LoadSound("Stagesep.wav");
	SLEVA = LoadSound("LEVA.wav");
	Scontact = LoadSound("contact.wav");
	Sclick = LoadSound(CLICK_SOUND);
	Bclick = LoadSound("button.wav");
	Gclick = LoadSound("guard.wav");
	CabinFans = LoadSound("cabin.wav");
	Vox = LoadSound("vox.wav");
	Afire = LoadSound("des_abort.wav");
}
...

{
	...
	case 0: // ilm panel
		...
		srf[6] = oapiCreateSurface (LOADBMP (IDB_ILMLEMSWITCH1));
		...
		srf[19] = oapiCreateSurface (LOADBMP (IDB_ILMLEMSWITCH2));
		srf[20] = oapiCreateSurface (LOADBMP (IDB_ILMLEMSWITCH3));
...

{
	...
	case 1:
		...
		oapiRegisterPanelArea (AID_DSKY,						_R(1343,  959, 1420, 1090), PANEL_REDRAW_ALWAYS, PANEL_MOUSE_DOWN,PANEL_MAP_BACKGROUND);
...
DLLCLBK VESSEL *ovcInit (OBJHANDLE hvessel, int flightmodel)
{
	...
	InitSoundLib(hvessel, "NASSP", 1, 60);
...
{
	sat5_lmpkd *lem = (sat5_lmpkd *)vessel;
	return lem->ConsumeKey(keystate);
}

int sat5_lmpkd::ConsumeKey (const char *keystate)
...
void sat5_lmpkd::Timestep(double simt)

{
	...
	}else if (stage == 1||stage == 5)
	{
		...
		if (ENGARMswitch && !ASCHE1switch && !ASCHE2switch && ED1switch && ED6switch && ED7switch && ED8switch){
			SetThrusterResource(th_hover[0],ph_Asc);
		}else{
			SetThrusterResource(th_hover[0],NULL);
		}
...
{
	sat5_lmpkd *lem = (sat5_lmpkd *) vessel;
	lem->LoadStateEx(scn, vs);
}

void sat5_lmpkd::LoadStateEx (FILEHANDLE scn, VESSELSTATUS *vs)
...
void sat5_lmpkd::SetLanderData(double latitude, double longitude, double altitude, int MissionNo)

{
	agc.SetApolloNo(MissionNo);
	agc.SetDesiredLanding(latitude, longitude, altitude);
}
