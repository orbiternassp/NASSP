...
The basic function of the DKI is to compute impulsive maneuvers; the result of these maneuver is the rendezvous of the CSM and LM spacecraft. The DKI attains a coelliptic orbit by doing three maneuvers: (1) phase, (2) height, (3) a coelliptic maneuver that puts the chaser in a coelliptic orbit with the target. From this orbit a terminal phase maneuver (TPI) and a terminal velocity match maneuver (TPF) maybe be performed to achieve the actual rendezvous.\\

Several plans are available:

\begin{itemize}
	\item CSI/CDH Sequence: DKI maneuver is phasing, followed by CSI, CDH and TPI.
	\item HAM-CSI/CDH Sequence: DKI maneuver is phasing, followed by a boost (10 ft/s), height, CSI, CDH and TPI maneuvers.
	\item Rescue-2 Sequence: DKI maneuver is height, followed by CSI, CDH and TPI.
	\item TPI Time Only: Calculates only the TPI time based on lighting.
	\item High Dwell Sequence: DKI maneuver is phasing, followed by height, CSI, CDH and TPI.
\end{itemize}

\paragraph{Buttons}\mbox{} \\

\textbf{INI:} Go to DKI initialization page.\\
\textbf{PRO:} Choose the rendezvous profile.\\
\textbf{CHA:} Choose which vehicle is chaser, and which is the target.\\
\textbf{VTI:} Choose time at which the state vectors should be taken from the ephemerides (MPT mode only)\\
\textbf{TIG:} Choose the time of ignition for the DKI maneuver.\\
\textbf{TPI:} Estimate for the time of the TPI maneuver.\\
\textbf{TGT:} Choose the other vehicle involved in the rendezvous sequence (non-MPT mode only)\\
\textbf{OPT:} Go to DKI options page.\\
...
\textbf{DH:} Delta height at the CDH maneuver.\\
\textbf{E:} Desired elevation angle at TPI.\\
\textbf{BCK:} Go back to DKI calculation page.\\

\paragraph{Options page buttons}\mbox{} \\

\textbf{MAN:} Maneuvers separated by half orbits (or multiples of them) or time differences.\\
\textbf{RAD:} Adds a 50 ft/s radial component for the burn, as done on some missions.\\
\textbf{TPI:} Cycle through TPI options.\\
\textbf{SUN:} Input time for TPI options.\\
\textbf{NHC:} Choose the number of half-revs between CSI and CDH.\\
\textbf{NPB:} Choose the number of half-revs between Phasing and Boost.\\
\textbf{DT1:} Choose the time between abort and Boost/CSI.\\
\textbf{DT2:} Choose the time between boost and HAM.\\
\textbf{DT3:} Choose the time between HAM and CSI.\\
\textbf{BCK:} Go back to the DKI page.\\
