...
This MFD page contains the three main calculation tools for rendezvous maneuvers, as well as a separate section for calculating TPI times:\\

\textbf{MS:} Two Impulse Processor - Multiple Solution.\\ 
\textbf{CC:} Two Impulse Processor - Corrective Combination.\\ 
\textbf{SS:}  Two Impulse Processor - Single Solution.\\ 
\textbf{SPQ:} Coelliptic Rendezvous Processor.\\
\textbf{DKI:} Docking initiate. Contains the inputs and display for the Docking Initiation Processor (DKI).\\ 
\textbf{TPI:} Calculate TPI times.\\ 
...
\newpage
\paragraph{Two Impulse Transfer}\mbox{} \\

The Two Impulse transfer page converts an impulsive maneuver to a finite burn with selected thruster and thrust profile.\\

\textbf{TAB:} Source for the maneuver solution. The two options are Multiple Solution or Corrective Combination.\\
\textbf{PLN:} Number of the plan from either of the two possible solutions.\\
\textbf{DEL:} If the MPT is active, choose the time after which all maneuvers in both MPTs are going to be deleted before the maneuvers are transferred.\\
 \textbf{MAN:} Choose the maneuver (1 or 2) for which the following inputs are then used in the transferred.\\
\textbf{THR:} Choose the thruster for the maneuver.\\
\textbf{ATT:} Choose the attitude mode for the maneuver. The possible options are AGS External DV, PGNS External DV, Manual or Inertial.\\
\textbf{ULL:} Choose the ullage options for the maneuver.\\
\textbf{ITE:} The iterate flag may be set to either iterate or do not iterate. If Do Not Iterate is selected, the maneuver will be transferred as is, taking into account the Time flag below. If Iterate is chosen, the maneuver's attitude and delta velocity will be altered such that the post-burn orbital elements match the calculated impulsive post-burn elements to within specified tolerances.\\
\textbf{10P:} If the maneuver is performed with the DPS, choose the duration for which 10\% thrust is used before throttle up. If this parameter is set to the negative of its desired value then the PGNS short burn test is ignored. The PGNS, when using auto throttle, normally decides for short burns to not throttle up from 10\% at all. This can be used to simulate a manual, early throttle up.\\
\textbf{DPS:} Choose the DPS thrust setting after throttle up.\\
\textbf{TIM:} The impulsive flag may be set to either Impulsive (IMP) or Optimize (OPT) to control TIG selection during the calculation of the actual TIG and DV. If Optimum is chosen, the maneuver's burn arc is taken into account by "splitting the TIG". That is, the transferred TIG is set equal to the impulsive TIG minus one-half of the length of the burn. The impulsive delta velocities, converted to the inertial frame, are then applied to the burn. When Impulsive is chosen, the transferred TIG is the same as the computed TIG, resulting in a net difference in LVLH burn components due to the finite burn arc. In normal practice, the impulsive flag is set to Optimum for any non-Lambert burns in order to center the burn arc about the impulsive TIG. This is extremely important for big burns ($\gg$ 50 fps) that have a significant burn arc.\\
\textbf{SAV:} Use the settings for the current maneuver and apply them to all to-be-transferred maneuvers.\\
\newpage
...
\newpage
\paragraph{SPQ Transfer Page}\mbox{} \\

The transfer page converts an impulsive maneuver to a finite burn with selected thruster and thrust profile.\\

\textbf{PLN:} Choose if a DKI, SPQ or lunar descent plan is supposed to be transferred. In the case of the DKI, the multiple available plans can be selected by inputting 1-7.\\
\textbf{DEL:} If the MPT is active, choose the time after which all maneuvers in both MPTs are going to be deleted before the maneuvers are transferred.\\
 \textbf{MAN:} Choose the maneuver in the plan for which the following inputs are then used in the transferred.\\
\textbf{THR:} Choose the thruster for the maneuver.\\
\textbf{ATT:} Choose the attitude mode for the maneuver. The possible options are AGS External DV, PGNS External DV, Manual or Inertial.\\
\textbf{ULL:} Choose the ullage options for the maneuver.\\
\textbf{ITE:} The iterate flag may be set to either iterate or do not iterate. If Do Not Iterate is selected, the maneuver will be transferred as is, taking into account the Time flag below. If Iterate is chosen, the maneuver's attitude and delta velocity will be altered such that the post-burn orbital elements match the calculated impulsive post-burn elements to within specified tolerances.\\
...
\newpage
\paragraph{DKI Transfer Page}\mbox{} \\

See SPQ Transfer Page.
\newpage
...
0&No error\\
1&Maneuver cannot be performed in this orbit\\
2&Maneuver cannot be performed at this point in the orbit\\
3&AEG failed to converge\\
4&Unrecoverable AEG error\\
5&Unable to obtain libration matrix\\
6&Unable to advance to selenographic argument of latitude\\
7&PMMAPD error for current apoapsis/periapsis\\
8&PMMAPD error for resultant apoapsis/periapsis\\ 
9&Resultant orbit non-elliptical\\
\end{tabular}
\newpage
\subsubsection{Transfer Page} 

The transfer page converts an impulsive maneuver to a finite burn with selected thruster and thrust profile.\\

\textbf{TAB:} Choose if the transfer is for the CSM or LM.\\
\textbf{REP:} Choose the maneuver to be replaced (if MPT is active).\\
\textbf{THR:} Choose the thruster for the maneuver.\\
\textbf{ATT:} Choose the attitude mode for the maneuver. The possible options are AGS External DV, PGNS External DV, Manual or Inertial.\\
\textbf{ULL:} Choose the ullage options for the maneuver.\\
\textbf{ITE:} The iterate flag may be set to either iterate or do not iterate. If Do Not Iterate is selected, the maneuver will be transferred as is, taking into account the Time flag below. If Iterate is chosen, the maneuver's attitude and delta velocity will be altered such that the post-burn orbital elements match the calculated impulsive post-burn elements to within specified tolerances.\\
\textbf{DT:} If the maneuver is performed with the DPS, choose the duration for which 10\% thrust is used before throttle up. If this parameter is set to the negative of its desired value then the PGNS short burn test is ignored. The PGNS, when using auto throttle, normally decides for short burns to not throttle up from 10\% at all. This can be used to simulate a manual, early throttle up.\\
\textbf{PCT:} Choose the DPS thrust setting after throttle up.\\
\textbf{TIM:} The impulsive flag may be set to either Impulsive (IMP) or Optimize (OPT) to control TIG selection during the calculation of the actual TIG and DV. If Optimum is chosen, the maneuver's burn arc is taken into account by "splitting the TIG". That is, the transferred TIG is set equal to the impulsive TIG minus one-half of the length of the burn. The impulsive delta velocities, converted to the inertial frame, are then applied to the burn. When Impulsive is chosen, the transferred TIG is the same as the computed TIG, resulting in a net difference in LVLH burn components due to the finite burn arc. In normal practice, the impulsive flag is set to Optimum for any non-Lambert burns in order to center the burn arc about the impulsive TIG. This is extremely important for big burns ($\gg$ 50 fps) that have a significant burn arc.\\
\newpage
...
\textbf{TAB:} Choose if the transfer is for the CSM or LM.\\
\textbf{REP:} Choose the maneuver to be replaced (if MPT is active).\\
\textbf{NUM:} Choose the solution to be transferred.\\
\textbf{THR:} Choose the thruster for the maneuver.\\
\textbf{ATT:} Choose the attitude mode for the maneuver. The possible options are AGS External DV, PGNS External DV, Manual or Inertial.\\
\textbf{UDT:} Choose the ullage options for the maneuver.\\
\textbf{ITE:} The iterate flag may be set to either iterate or do not iterate. If Do Not Iterate is selected, the maneuver will be transferred as is, taking into account the Time flag below. If Iterate is chosen, the maneuver's attitude and delta velocity will be altered such that the post-burn orbital elements match the calculated impulsive post-burn elements to within specified tolerances.\\
\textbf{DT:} If the maneuver is performed with the DPS, choose the duration for which 10\% thrust is used before throttle up. If this parameter is set to the negative of its desired value then the PGNS short burn test is ignored. The PGNS, when using auto throttle, normally decides for short burns to not throttle up from 10\% at all. This can be used to simulate a manual, early throttle up.\\
\textbf{PCT:} Choose the DPS thrust setting after throttle up.\\
\textbf{TIM:} The impulsive flag may be set to either Impulsive (IMP) or Optimize (OPT) to control TIG selection during the calculation of the actual TIG and DV. If Optimum is chosen, the maneuver's burn arc is taken into account by "splitting the TIG". That is, the transferred TIG is set equal to the impulsive TIG minus one-half of the length of the burn. The impulsive delta velocities, converted to the inertial frame, are then applied to the burn. When Impulsive is chosen, the transferred TIG is the same as the computed TIG, resulting in a net difference in LVLH burn components due to the finite burn arc. In normal practice, the impulsive flag is set to Optimum for any non-Lambert burns in order to center the burn arc about the impulsive TIG. This is extremely important for big burns ($\gg$ 50 fps) that have a significant burn arc.\\
...
\newpage
\subsubsection{Transfer Page} 
See Midcourse Correction Processor Transfer Page.\\
\newpage
...
\underline{Code}&\underline{Error Description}\\
0&No error\\
1&$\Delta$V of main engine $\le$ 0\\
2&Coast numerical integrator failed backing up to ignition time\\
6&Forward iterator failed to converge\\
7&Error in conversion from ECI to ECT coordinates\\
8&Coast numerical integrator failed in PMMATC\\
9&Maneuver numerical integrator failed in PMMATC\\
10&REFSMMAT not available\\
11&Illegal maneuver code\\
12&Invalid vehicle configuration\\
13&Invalid number of ullage quads\\
14&Invalid entry mode\\
\end{tabular}
\newpage
...
\newpage
\subsection{Perigee Adjust}
The function of the Perigee Adjust Processor is to compute a series of maneuvers, each of which will raise perigee to a specified height while keeping the thrust direction at a constant angle to the horizon. A threshold time, desired perigee height, and a desired time increment are the main inputs; while the main display values are the begin-burn time, the required velocity increment, and the resultant apogee and perigee values for each of the six maneuvers computed. A table containing the elements before and after each of the maneuvers is also provided so that a selected maneuver can be transferred if so desired.\\
\subsubsection{Inputs}
\textbf{VEH:} Calculate the maneuvers for the CSM or for the LM.\\
\textbf{VEC:} If the MPT is active, select the vector time.\\
\textbf{THT:} Threshold time for the maneuver calculations. This time will be the time of the first maneuver.\\
\textbf{INC:} Time increment between maneuvers.\\
\textbf{HP:} Desired periapsis altitude to which the maneuvers will raise the orbit.\\
\textbf{VID:} Vector ID. If something is entered here then the calculation will try to use this state vector from the Vector Panel Summary instead of the current trajectory or ephemeris of the selected vehicle.\\
\subsubsection{Display}
\textbf{TIG:} Time of ignition in Ground Elapsed Time.\\
\textbf{DV:} Delta V of the maneuver in feet per second.\\
\textbf{BT:} Burn time of the maneuver.\\
\textbf{TAA:} True anomaly at TIG in degrees.\\
\textbf{H:} Height at TIG in nautical miles.\\
\textbf{HA:} Height of apoapsis after the maneuver in nautical miles.\\
\newpage
...
\newpage
\section{Uplinks}
A variety of AGC and LVDC uplinks are supported by the RTCC MFD.  These are selected by their load type number. Here is a list of the available uplinks:\\

CMC:
\begin{itemize}
\item 00: CSM Navigation Update
\item 06: Landing Site Vector
\item 07: Time Increment
\item 08: Liftoff Time Update
\item 09: LM Navigation Update
\item 10: External Delta-V Update
\item 12: REFSMMAT Update
\item 13: Retrofire External Delta-V Update
\item 14: Entry Update
\item 18: Erasable Memory Update A
\item 19: Erasable Memory Update B
\end{itemize}
LGC:\\
\begin{itemize}
\item 20: LM Navigation Update
\item 21: CSM Navigation Update
\item 22: External Delta-V Update
\item 23: REFSMMAT Update
\item 24: Time Increment
\item 25: Liftoff Time Update
\item 26: Landing Site Vector
\item 27: Descent Update
\item 28: Erasable Memory Update A
\item 29: Erasable Memory Update B
\end{itemize}
\newpage
LVDC:\\
\begin{itemize}
\item 49: LVDC Navigation Update
\end{itemize}
\newpage
\subsection{AGC Navigation Update}
\subsubsection{Explanation}
The state vector of the vessel can be calculated and uplinked here. The AGC has two slots for state vectors: CSM and LM.  Both CMC and LGC have these two slots.
\subsubsection{Buttons}
\textbf{TIM:} Set the desired GET for the state vector.\\
\textbf{TGT:} Set the target vessel.\\
\textbf{CLC:} Calculate a state vector.\\
\textbf{UPL:} Uplinks the calculated data to the AGC.\\
\textbf{BCK:} Go back to the main menu.\\
\subsection{External Delta-V Uplink}
\subsubsection{Explanation}
Uplink for burn data calculated with any of the RTCC maneuver targeting processors.
\subsubsection{Buttons}
\textbf{CLC:} Calculate the uplink data.\\
\textbf{UPL:} Uplinks the calculated data to the AGC.\\
\textbf{BCK:} Go back to the main menu.\\
\subsection{Time Increment}
\subsubsection{Explanation}
Uplink to adjust the CMC or LGC clock. Due to clock drift the clock could become misaligned with the actual mission elapsed time. This MFD page also contains a little utility calculation to calculate the current clock error. This calculated time can then be input for the uplink.
\subsubsection{Buttons}
\textbf{CLC:} Calculate the uplink data.\\
\textbf{UPL:} Uplinks the calculated data to the AGC.\\
\textbf{BCK:} Go back to the main menu.\\
\subsection{Liftoff Time Update}
\subsubsection{Explanation}
Uplink to adjust the time reference in the AGC. This will change the liftoff time (TEPHEM), the state vector times and the AGC clock. The MFD page has a calculation utility to calculate the change in liftoff time required to get to a specific liftoff time reference.\\
\subsubsection{Buttons}
\textbf{CLC:} Calculate the uplink data.\\
\textbf{UPL:} Uplinks the calculated data to the AGC.\\
\textbf{BCK:} Go back to the main menu.\\
\subsection{REFSMMAT Uplink}
\subsubsection{Explanation}
Uplink to the AGC to change the REFSMMAT. There are two types of REFSMMAT uplinks. The actual REFSMMAT can be changed directly, which is usually only desired on an initial LM activation. The usual case is to uplink a desired REFSMMAT, which can then get used in a P52 option 1 to realignment the IMU from a previously already existing alignment.
\subsubsection{Buttons}
\textbf{CLC:} Calculate the uplink data.\\
\textbf{UPL:} Uplinks the calculated data to the AGC.\\
\textbf{BCK:} Go back to the main menu.\\
\subsection{Descent Update}
\subsubsection{Explanation}
The LGC only has a lunar landing time stored that is used as an initial guess for the P63 ignition algorithm. The time can be uplinked with this MFD page.
\subsubsection{Buttons}
\textbf{CLC:} Calculate the uplink data.\\
\textbf{UPL:} Uplinks the calculated data to the AGC.\\
\textbf{BCK:} Go back to the main menu.\\
...
\underline{Code}&\underline{Error Description}\\
0&No error\\
1&The trajectory for subject vehicle is not generated\\
2&The Mission Plan table is either in an update state or inconsistent with trajectory update number\\
3&The revolution defining times table is either in an update state or is inconsistent with trajectory update number\\
4&The main ephemeris is in update status\\
5&The revolution number on the MED U13 is less than the first available revolution defining time\\
6&The revolution number on the MED U13 is greater than the last available revolution defining time\\
7&The revolution number on the U13 MED is valid; however, the longitude crossing requested is outside the range of the ephemeris\\
8&The time on the U14 MED precedes the start of the ephemeris\\
9&The time on the U14 MED exceeds the end of the ephemeris\\
10&The revolution number on the U12 MED is smaller than the first available revolution defining time\\
11&The revolution number on the U12 MED is greater than the last revolution defining time\\
12&The time on the U12 MED precedes the start of the ephemeris\\
13&The time on the U12 MED exceeds the end of the ephemeris\\
14&The maneuver number requested is greater than the number of maneuvers in the Mission Plan Table\\
15&The base vector selected for predicted apogee/perigee computation was rejected by the Analytical Ephemeris Generator\\
16&The base vector selected for upcoming apogee/perigee computation was rejected by the Analytical Ephemeris Generator\\
17&No precession/nutation matrix is available for subject time\\
18&Open\\
19&Present position data not initialized in Time Constraints Table\\
20&End of ephemeris reached\\
\end{tabular}
\newpage
...
\underline{Code}&\underline{Error Description}\\
0&No error\\
1&The display has not been initialized via MED U00\\
2.&he trajectory for subject vehicle is not generated\\
3&The time on the U01 exceeds the end of the ephemeris\\
4&The time on the U01 precedes the start of the ephemeris\\
5&The main ephemeris is in an update status\\
6&The Mission Plan Table is either in an update state or is inconsistentwith the trajectory update number\\
7&The time on the U01 falls within a maneuver\\
8&The maneuver number requested is greater than the number of maneuvers in the Mission Plan Table\\
9&Maneuver on U01 not integrated through\\
10&No precession/nutation is available for subject time\\
11&Base vector rejected by Encke Integrator\\
12&The base vector selected for apogee/perigee computation was rejected by the analytical ephemeris generator\\
\end{tabular}
\newpage
...
\textbf{INST ID:} Instrument ID. The last 3 characters must be CSM or LEM.\\
 \textbf{TARGET NM:} Target code. 5 to 8 characters have to be input.  Mode 1: The last character is E or M. Mode 3: The last 3 characters are a valid star number. Mode 4: Enter 7 characters. Last 3 are CSM or LEM. The first 4 spaces in the TARGET code are for experiment identification and no computations are based upon them.\\
...
\end{tabular}
\newpage
\subsection{Checkout Monitor (MSK 1619)}
\subsubsection{Function}
The function of the Checkout Monitor Display, EMDCHECK, is to obtain or calculate various orbital parameters, system parameters, and mission planning quantities to aid in the RTCC program checkout.
\newpage
\subsubsection{Display Parameters}
\begin{center}
\begin{tabular} { L{2cm} L{8cm} L{6cm} }
\underline{Quantity}&\underline{Definition}&\underline{Dimensions}\\
VEH&Vehicle associated with parameters&\\
GET&Ground elapsed time tag of the vector&hhh:mm:ss.ss\\
GMT&Greenwich mean time tag of the vector&hhh:mm:ss.ss\\
VID&Identification of the last vector used to update the ephemeris&\\
X&&\\
Y&Position vector&feet or Earth radii\\
Z&&\\
$\dot X$&\\
$\dot Y$&Velocity vector&FPS or Earth radii per hour\\
$\dot Z$&\\ 
V\textsubscript{i} &Inertial velocity&FPS\\
$\gamma$\textsubscript{i}&Inertial flight-path angle&deg.\\
$\psi$&Inertial azimuth&deg.\\
$\phi$\textsubscript{c}&Geocentric/selenographic latitude&deg.\\
$\lambda$&Geographic/selenographic longitude&deg.\\
h\textsubscript{s}&Height of vehicle above a spherical Earth or moon&NM\\
h\textsubscript{a}&Height of apogee above a spherical Earth or moon&NM\\
h\textsubscript{p}&Height of perigee above a spherical Earth or moon&NM\\
h\textsubscript{o}&Height above an oblate Earth&NM\\
h\textsubscript{o}&Height above an oblate Earth&ft.\\
$\phi$\textsubscript{D}&Geodetic/selenographic latitude&deg.\\
$\lambda$\textsubscript{D}&Mean geographic/true inertial longitude&deg.\\
a&Semi-major axis of orbital ellipse&NM\\
e&Eccentricity of orbital ellipse&\\
i&Orbital inclination&deg.\\
$\omega$\textsubscript{p}&Argument of perigee&deg.\\
$\Omega$&Right ascension of the ascending node (inertial)&deg.\\
$\nu$&True anomaly&deg.\\
m&Mean anomaly&deg.\\
R&Vector magnitude radius&NM\\
R-DAY&Calendar day of liftoff&day:month:year\\
A&Vehicle area at GET&sq. ft.\\
K-FAC&Current atmospheric density multiplier\\
CFG&Vehicle configuration at GET&\\
RF&Reference frame the vector is in: ECT = true Earth, ECI = mean Earth, MCT = true moon, MCI = mean moon&\\
\end{tabular}
\end{center}
\newpage
\begin{center}
\begin{tabular} { L{2cm} L{8cm} L{6cm} }
\underline{Quantity}&\underline{Definition}&\underline{Dimensions}\\
WT&Total weight for above configuration&lb.\\
WC&Total CSM weight&lb.\\
WL&Total LM weight&lb.\\
SPS&Total SPS propellant remaining&lb.\\
RCS\textsubscript{C}&Total CSM RCS propellant remaining&lb.\\
APS&Total APS propellant remaining&lb.\\
DPS&Total DPS propellant remaining&lb.\\
RCS\textsubscript{L}&Total LM RCS propellant remaining&lb.\\
J2&Total J2 propellant remaining&lb.\\
LO/C&Lift-off time of the CSM&hhh:mm:ss.ss\\
GRR/C&Guidance reference release time of the spacecraft&hhh:mm:ss.ss\\
ZS/C&Time that S/C register was zeroed&hhh:mm:ss.ss\\
GRR/S&Guidance reference release time of the S-IVB&hhh:mm:ss.ss\\
LO/L&Lift-off time of the LM&hhh:mm:ss.ss\\
ZS/L&Time that the LGC register was zeroed&hhh:mm:ss.ss\\
ZS/A&Time that the AGS register was zeroed&hhh:mm:ss.ss\\
EPHB&The begin time of the ephemeris&hhh:mm:ss.ss\\
EPHE&The end time of the ephemeris&hhh:mm:ss.ss\\
Update No.&To indicate the number of times that the ephemeris was updated&\\
NV&Number of vectors used to interpolate for vector displayed&\\
THT&MED threshold time&hhh:mm:ss.ss\\
RTT&Reentry threshold time&hhh:mm:ss.ss\\
LSB&Lunar stay begin time&hhh:mm:ss.ss\\
LLS&Lunar stay termination&hhh:mm:ss.ss\\
$\delta$L&Lunar declination&deg.\\
VSF&Venting scale factor&\\
EB2&Ephemeris begin time (other vehicle)&hhh:mm:ss.ss\\
EE2&Ephemeris end time (other vehicle)&hhh:mm:ss.ss\\
X\textsubscript{T}&Unit thrust vector assocated with&\\
Y\textsubscript{T}&the requested maneuver&\\
Z\textsubscript{T}& &\\
Option&MED option to which displayed data corresponds (GET, GMT, MVI, MVE, RAD, ALT, FPA or LNG)&\\
\end{tabular}
\end{center}
\newpage
\subsubsection{Errors}
\begin{tabular}{p{3cm} p{10cm}}
\underline{Code}&\underline{Error Description}\\
1&MED input is required.\\
2&The Mission Plan Table is in an update state.\\
3&Update number are inconsistent across trajectory related tables.\\
4&Input time is outside of ephemeris range.\\
5&Input time is during a maneuver.\\
6&Open.\\
7&Requested maneuver is not in MPT.\\
8&No ephemeris is available for subject vehicle (Zero update number is in MPT.)\\
9&Precession/nutation matrix is not available for time of concern\\
10&End conditions not achieved within integration time limits.\\
11&Base vector rejected by analytical ephemeris generator.\\
12&Base vector rejected by Encke Integrator.\\
\end{tabular}
\newpage
...
\hline
H\_A\_LPO1 &Apolune height on LPO-1 (orbit after LOI). Used by TLMCC and LOI targeting&170.0& Nautical miles\\
\hline
H\_P\_LPO1 &Perilune height on LPO-1 (orbit after LOI). Used by TLMCC and LOI targeting&60.0& Nautical miles\\
\hline
H\_A\_LPO2 &Apolune height on LPO-2 (orbit after DOI). Used by TLMCC and LOI targeting&60.0& Nautical miles\\
\hline
H\_P\_LPO2 &Perilune height on LPO-2 (orbit after DOI). Used by TLMCC and LOI targeting&8.23& Nautical miles\\
\hline
